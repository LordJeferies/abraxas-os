...
on_error() {
  ...
  # IMPORTANT:
  # Use this checkout's ROOT. Never hardcode ~/Desktop/Abrxs os.
...
python3 "$ROOT/scripts/check_generated_state_coherence.py"
...
# IMPORTANT:
# Update lifecycle state in THIS checkout, not in a hardcoded desktop repo.
python3 - "$ROOT" <<'PY'
from pathlib import Path
from datetime import datetime
import json
